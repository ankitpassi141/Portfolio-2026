// Case-study sheet (dialog) shared by the Home page and the All Case
// Studies page. Full write-ups live in CASES below — edit the text
// directly. If js/case-studies-data.js has a real URL for the same id,
// that takes over entirely and the click opens it instead of this sheet
// (see externalCaseUrl). Requires a <div class="sheet" id="sheet">...
// element to exist on the page (see index.html / case-studies.html) —
// pages without one just skip this file's setup.
(() => {
  "use strict";
  ...
  // Wires every [data-case] element to open the sheet (or, if
  // case-studies-data.js has a real URL, to link straight out to it
  // instead). Exposed on window and re-run by main.js after the drawer
  // renders, since that adds more [data-case] elements after this file's
  // own initial pass has already run — the "wired" guard keeps it from
  // double-binding anything.
  function wireCaseLinks() {
    document.querySelectorAll("[data-case]:not([data-case-wired])").forEach((el) => {
      el.setAttribute("data-case-wired", "");
